...
export function shadows(colorScheme: 'light' | 'dark'): Shadows {
  const colorChannel = colorScheme === 'light' ? color.grey[500] : color.grey[900]

  const color1 = alpha(colorChannel, 0.2)
  const color2 = alpha(colorChannel, 0.14)
  const color3 = alpha(colorChannel, 0.12)
